...
__export(index_exports, {
  CryptoAssertError: () => CryptoAssertError,
  compare: () => compare,
  hash: () => hash
});
...
function encodeUtf8(value) {
  ...
  for (let i = 0; i < stringLength; i++) {
    ...
    if (codePoint < 128) {
      ...
      if (codePoint < 2048) {
        buffer[w++] = codePoint >> 6 & 31 | 192;
      } else {
        if (codePoint < 65536) {
          buffer[w++] = codePoint >> 12 & 15 | 224;
        } else {
          buffer[w++] = codePoint >> 18 & 7 | 240;
          buffer[w++] = codePoint >> 12 & 63 | 128;
        }
        buffer[w++] = codePoint >> 6 & 63 | 128;
      }
      buffer[w++] = codePoint & 63 | 128;
    }
  }
...
var CryptoAssertError = class _CryptoAssertError extends Error {
  actual;
  expected;
  constructor(message, actual, expected) {
    ...
    Object.setPrototypeOf(this, _CryptoAssertError.prototype);
  }
};
...
function tse(a, b) {
  ...
  let viewA = a instanceof DataView ? a : new DataView(ArrayBuffer.isView(a) ? a.buffer : a);
  let viewB = b instanceof DataView ? b : new DataView(ArrayBuffer.isView(b) ? b.buffer : b);
  let result = 0;
...
var BCRYPT_BASE64_ALPHABET = "./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
function encodeBase64(buffer, length) {
  let off = 0;
  let rs = "";
  while (off < buffer.length) {
    let c1 = buffer[off++];
    rs += BCRYPT_BASE64_ALPHABET[c1 >> 2 & 63];
    c1 = (c1 & 3) << 4;
    if (off >= buffer.length) {
      rs += BCRYPT_BASE64_ALPHABET[c1 & 63];
      break;
    }
    let c2 = buffer[off++];
    c1 |= c2 >> 4 & 15;
    ...
    c1 |= c3 >> 6 & 3;
    rs += BCRYPT_BASE64_ALPHABET[c1 & 63];
    rs += BCRYPT_BASE64_ALPHABET[c3 & 63];
  }
...
function cryptRaw(rounds, salt, password) {
  const cdata = new Uint32Array(CIPHER_TEXT);
  ...
  try {
    ...
    ret[0] = cdata[0] >>> 24 & 255;
    ret[1] = cdata[0] >>> 16 & 255;
    ret[2] = cdata[0] >>> 8 & 255;
    ret[3] = cdata[0] & 255;
    ret[4] = cdata[1] >>> 24 & 255;
    ret[5] = cdata[1] >>> 16 & 255;
    ret[6] = cdata[1] >>> 8 & 255;
    ret[7] = cdata[1] & 255;
    ret[8] = cdata[2] >>> 24 & 255;
    ret[9] = cdata[2] >>> 16 & 255;
    ret[10] = cdata[2] >>> 8 & 255;
    ret[11] = cdata[2] & 255;
    ret[12] = cdata[3] >>> 24 & 255;
    ret[13] = cdata[3] >>> 16 & 255;
    ret[14] = cdata[3] >>> 8 & 255;
    ret[15] = cdata[3] & 255;
    ret[16] = cdata[4] >>> 24 & 255;
    ret[17] = cdata[4] >>> 16 & 255;
    ret[18] = cdata[4] >>> 8 & 255;
    ret[19] = cdata[4] & 255;
    ret[20] = cdata[5] >>> 24 & 255;
    ret[21] = cdata[5] >>> 16 & 255;
    ret[22] = cdata[5] >>> 8 & 255;
    ret[23] = cdata[5] & 255;
    return ret;
  } finally {
    bf.dispose();
...
function compare(password, hash2) {
  cassert(
    /^\$2[aby]\$\d\d\$[./A-Za-z0-9]{53}$/.test(hash2),
    "Invalid bcrypt hash format."
  );
  const costStr = hash2.substring(4, 6);
  const cost = parseInt(costStr, 10);
  cassert(
    cost >= MIN_COST && cost <= MAX_COST,
    `Cost factor must be between ${MIN_COST} and ${MAX_COST}.`
  );
  const saltEncoded = hash2.substring(7, 29);
  const expectedHash = hash2.substring(29);
  const passwordBytes = encodeUtf8(password);
  cassert(passwordBytes.length > 0, "Password must not be empty.");
  cassert(
    passwordBytes.length + 1 <= MAX_PW_LENGTH,
